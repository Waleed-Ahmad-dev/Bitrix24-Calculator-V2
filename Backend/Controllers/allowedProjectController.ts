...
export const getAllowedProjects = async (
  req: Request,
  res: Response,
): Promise<void> => {
  ...
  const currentUserDataRawResponse = await client.actions.v2.call.make({
    method: "user.get",
    params: { filter: { ID: userId } },
    idKey: "ID",
    requestId: `currentUserData-${Date.now()}`,
  });

  const currentUserData: Record<string, any> =
    (currentUserDataRawResponse.getData() as any)?.result?.[0] || {};
  ...
  if (currentUserProjectArray.length === 0) {
    res
      .status(200)
      .json({
        message: "No projects assigned to the user",
        allowedProjects: [],
      });
    return;
  }

  const projectList = await client.actions.v2.call.make({
    method: "user.userfield.list",
    params: { filter: { ID: 489 } },
    idKey: "ID",
    requestId: `projectList-${Date.now()}`,
  });
